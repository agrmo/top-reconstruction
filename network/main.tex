...
\g{k = \bk \pm \sqrt{\bk}}
\begin{center}
Nodes never deviate significantly from $\bk$.\\~\\
\textit{``scale''}
\end{center}
\end{frame}
...
\begin{itemize}
\item Each node has a fixed number of edges.
\item Each edge is randomly placed.
\end{itemize}
\item Power Law Network
\end{itemize}
\end{frame}
...
\begin{itemize}
\item How many links do you have? $\bk$
\end{itemize}
...
It is most likely to connect to a node which already has $\bk$ links.\\~\\
...
\begin{frame}
\g{q(k)=Akp_k}
\g{\int_0^\infty Akp_k dk = 1}
\g{q(k)=Akp_k=AkCk^{-\gamma}}
\end{frame}
