...
\title{Renormalization Group Flows of Fermi Liquid Theory}
...
Landau's Fermi Liquid theory is a powerful tool for studying the interactions of electrons in the weak interaction and zero temperature limit. By studying the renormalization group flow of the interactions \ref{RevModPhys.66.129}, we can better understand which interactions become stronger or weaker at difference scales. This paper studies the renormalization group flow of Fermi Liquid Theory. We will work in one dimension and assuming only one band exists. The Hamiltonian for the fermi liquid will be described as
...
\subsection{Fixed Point Action}
We start by introducing the action and enforcing that the RG flow is a fixed point for the free action. Later we will try to learn if interactions move us away from the fixed point.
...
and therefore the variable redefinition is simply
...
\g{\prod_4 \br{dk'}{s} \prod_4 \br{d\omega'}{s} s \delta(k') s\delta(\omega')}
...
In other words the quartic contribution to the action is marginal at tree level. We should push on to the loop level to see the quadratic contribution to the action.
\subsection{One loop corrections: $\mu$}
The one loop corrections will renormalize the chemical potential $\mu$ and the quartic interaction term $u_0$. Different diagrams apply to $\mu$ and $u_0$. Because $\mu$ appears in the two-point interaction, the only one-loop diagram that renormalizes it is the tadpole diagram,
\figpic{2cm}{1.png}{Tadpole.}
The tadpole diagram has one unknown momentum that must be integrated over. The loop momentum is restricted to the region $\Lambda/s \leq |k| \leq \Lambda$. The $\omega$ must also be integrated over, which can be done using the Green's function for the loop, which is
\g{\br{1}{i\omega-k}}
The propagator has a pole at $i\omega$ and so the contour must be deformed, say, by the factor $e^{i\omega 0^+}$. Putting it together, the contribution of the tadpole diagram to $\mu$, after rescaling by $s$, is
\g{\mu'&=s(\mu - u_0\int_{-\infty}^\infty \br{d\omega}{2\pi} \int_{\br{\Lambda}{s}< |k| < \Lambda} \br{dk}{2\pi} \br{i\omega-k} e^{i\omega 0^+} )\\
&= s(\mu - u_0  \int_{\br{\Lambda}{s}< |k| < \Lambda} \br{dk}{2\pi} \theta(-k))\\
&= s(\mu-\br{u_0}{2\pi}(\Lambda-\br{\Lambda}{s}))\\
&= s(\mu-\br{u_0 \Lambda}{2\pi})+\br{u_0\Lambda}{2\pi}}
There is a value of $\mu$ which does not flow. It's the fixed point, and can be read off directly,
\g{\mu^*=\br{u_0\Lambda}{2\pi}}
We may also calculate the $\beta$ of $\mu$. Define $s=1+t$ for an infinitesimal $t$, then,
\g{\mu'&=(1+t)(\mu-\br{u_0\Lambda}{2\pi}(1-\br{1}{1+t})\\
&= \mu+t(\mu-\br{u_0\Lambda}{2\pi})}
And the $\beta$ flow of $\mu$ is the $t$ derivative,
\g{\dd{t}\mu=\mu-\br{u_0\Lambda}{2\pi}}
This concludes the analysis of $\mu$ at one loop.
\section{Conclusion}
The RG flow of Fermi Liquid Theory was studied for the chemical potential $\mu$ and the four-point coupling $u$. The procedure we followed was to first define an RG flow such that the free field theory was a Gaussian fixed point. Then we evaluated the 2-point interaction and 4-point interaction, rescaled the coupling parameters, and then rescaled the fields, to study the RG flow of the couplings $\mu$ and $u$. We first studied the quartic 4-point interaction and found it is marginal. We then studied the quadratic interaction at tree level to find it is relevant and must be renormalized at loop level, and at one loop, there is indeed a fixed point.\end{document}
